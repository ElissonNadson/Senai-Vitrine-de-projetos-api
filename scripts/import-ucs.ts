...
async function run(): Promise<void> {
  ...
  const dataFilePath = path.join(DATA_DIR, 'UCs_Presencial.xlsx');

  const ucUuidCache = new Map<string, string>();
  ...
  try {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      const cursoRaw = getFirstAvailableValue(row, ['Curso', 'CURSO']);
      const ucRaw = getFirstAvailableValue(row, [
        'Unidade_Curricular',
        'UNIDADE_CURRICULAR',
      ]);
      ...
      if (!cursoRaw || !ucRaw) {
        ...
        if (!cursoRaw) missingFields.push('curso');
        if (!ucRaw) missingFields.push('uc');

        const fileName = path.basename(dataFilePath);
        const lineNumber = index + 2; // +1 pelo índice e +1 pelo cabeçalho
        console.warn(
          `[SKIP] arquivo=${fileName} linha=${lineNumber} curso=${cursoRaw || 'N/A'} uc="${ucRaw || 'N/A'}" motivo=campos_obrigatorios_ausentes (${missingFields.join(', ')})`,
        );
        skippedRows += 1;
        continue;
      }

      // unidades curriculares: verifica por curso + nome
      const ucCacheKey = `${cursoRaw}|${ucRaw}`;
      let ucUuid = ucUuidCache.get(ucCacheKey);
      if (!ucUuid) {
        let courseUuid = courseUuidCache.get(cursoRaw);
        if (!courseUuid) {
          ...
          if (existingCourse.rowCount) {
            courseUuid = existingCourse.rows[0].uuid;
            reusedCourses += 1;
          } else {
            courseUuid = uuidv4();
            await client.query(
              `INSERT INTO cursos (uuid, nome) VALUES ($1, $2)`,
              [courseUuid, cursoRaw],
            );
            createdCourses += 1;
          }

          courseUuidCache.set(cursoRaw, courseUuid);
        }

        const existingUc = await client.query<{ uuid: string }>(
          `SELECT uuid FROM unidades_curriculares WHERE curso_uuid = $1 AND nome = $2 LIMIT 1`,
          [courseUuid, ucRaw],
        );

        if (existingUc.rowCount) {
          ucUuid = existingUc.rows[0].uuid;
          reusedUcs += 1;
        } else {
          ucUuid = uuidv4();
          await client.query(
            `INSERT INTO unidades_curriculares (uuid, curso_uuid, nome) VALUES ($1, $2, $3)`,
            [ucUuid, courseUuid, ucRaw],
          );
          createdUcs += 1;
        }
        ucUuidCache.set(ucCacheKey, ucUuid);
      }
    ...
    console.log(`Cursos reaproveitados: ${reusedCourses}`);
    console.log(`Unidades curriculares criadas: ${createdUcs}`);
    console.log(`Unidades curriculares reaproveitadas: ${reusedUcs}`);
    console.log(
